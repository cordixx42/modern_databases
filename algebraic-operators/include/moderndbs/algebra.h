...
struct RegisterPtrVectorHasher {
   uint64_t operator()(const std::vector<Register*>& registers) const {
      std::string hash_values;
      for (auto& reg : registers) {
         uint64_t hash = (*reg).get_hash();
...
class Select
   ...
   std::variant<PredicateAttributeInt64, PredicateAttributeChar16, PredicateAttributeAttribute> pred;
   std::vector<Register*> regs;
...
class Sort
   ...
   std::vector<std::vector<Register*>> all_materialized;
...
class HashJoin
   ...
   // could have also used std::unordered_map<Register, std::vector<Register>, RegisterHasher>

   // for left outer join, add boolean marker indicating if there is join partner

   std::unordered_map<Register, std::vector<Register*>, RegisterHasher> registersLeft;
   std::vector<Register*> registersRight;
...
class HashAggregation
   ...
   std::unordered_map<Register, std::vector<Register*>, RegisterHasher> res;

   std::unordered_map<std::vector<Register>, std::vector<Register*>, RegisterVectorHasher> res_general;

   std::vector<Register*> new_aggregate_registers;
...
class Union
   ...
   private:
   // TODO: add your implementation here
   std::unordered_set<std::vector<Register>, RegisterVectorHasher> encountered;
   std::vector<Register*> output;
...
class IntersectAll
   ...
   std::unordered_map<std::vector<Register>, uint32_t, RegisterVectorHasher> encountered_left_to_count;
   std::vector<Register*> output;
   std::vector<Register*> to_free;
...
class Except
   : public BinaryOperator {
   private:
   std::unordered_set<std::vector<Register>, RegisterVectorHasher> encountered_right;
   std::vector<Register*> output;
   std::vector<Register*> to_free;
...
class ExceptAll
   ...
   std::unordered_map<std::vector<Register>, uint32_t, RegisterVectorHasher> encountered_right_to_count;
   std::vector<Register*> output;
   std::vector<Register*> to_free;
